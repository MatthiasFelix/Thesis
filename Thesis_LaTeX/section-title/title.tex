\begin{titlepage}

\setlength{\textwidth}{16cm}
\setlength{\oddsidemargin}{0cm}
\setlength{\evensidemargin}{0cm}
\setlength{\topmargin}{8mm}
\setlength{\headheight}{0cm}
\setlength{\headsep}{0cm}
\setlength{\topskip}{0cm}
\enlargethispage{5cm}

\noindent
\setlength{\unitlength}{1mm}

\begin{picture}(160,226)
\centering

\put(0,203){\line(1,0){160}} % lower line
\put(109,39){\line(1,0){50}}
\put(109,161){\line(1,0){50}}
\put(109,170){\line(1,0){50}}
\put(107,1){\line(0,1){200}}

\put(0,140){\parbox[b]{100mm}{
    \begin{center}
    {\bf\Huge {\sffamily{Social Recommender Systems}}}
    \end{center}}}


\put(109,92){\parbox[b]{50mm}{
    {\bf
    {\sffamily{{\Large Matthias Felix}}}}\\
    {\sffamily{of Zurich, Switzerland\\\\
    Student-ID: 11-746-625\\
    matthias.felix@uzh.ch
    }}
}}

\put(109,161){\makebox(50,9){{\sffamily{Thesis \hfill\today}}}}

\put(0,215){\makebox(80,11)[l]{\includegraphics[height=2.8cm]
{./section-title/figures/uzh_logo_e_pos}}}


\put(109,185){\parbox[b]{55mm}{\flushleft
    {\bf{\sffamily{ Computation and Economics Research Group}}} 
    }
}

\put(109,32){\parbox[b]{50mm}{\flushleft
    {\sffamily{Advisor:}} 
    }
}

\put(109,10){\parbox[b]{50mm}{\flushleft
        {\sffamily{
        Prof. Sven Seuken, PhD\\
        Institut f\"ur Informatik\\
        Universit\"at Z\"urich\\
        http://www.ifi.uzh.ch/ce}
        }
}}

\end{picture}
\end{titlepage}

